...
-(void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations{
    ...
    notify_register_dispatch("com.apple.springboard.lockstate", &notify_token,dispatch_get_main_queue(), ^(int token) {
        ...
        if(state == 0) {
            //NSLog(@"unlock device");
            [viewController unlocked];
        } else {
            //NSLog(@"lock device");
            [viewController locked];
        }
        
        //NSLog(@"com.apple.springboard.lockstate = %llu", state);
    });
    [viewController update];
    // To ensure that the entire update code runs before the background process changes
    ...
    self.shareModel.delay10Seconds = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(stopLocationDelayBy10Seconds) userInfo:nil repeats:NO];
